...
public class FeetControl : MonoBehaviour
{
	...
	private float trackerDisplacement; // offset of tracker position from ground in RW (used shoe model y val as ref )
	...
	void FixedUpdate()
    {
		...
		{
            L_Foot_Model.transform.position = new Vector3(L_Tracked_Object.position.x, hitLeft.point.y, L_Tracked_Object.position.z);
			Debug.DrawLine(L_Tracked_Object.transform.position + bufferHeight, hitLeft.point, Color.red);
		}
		else
		{
            L_Foot_Model.transform.position = new Vector3(L_Tracked_Object.position.x, frozenFloorHeight, L_Tracked_Object.position.z);
		}
		...
		{
            R_Foot_Model.transform.position = new Vector3(R_Tracked_Object.position.x, hitRight.point.y, R_Tracked_Object.position.z);
			Debug.DrawLine(R_Tracked_Object.transform.position + bufferHeight, hitRight.point, Color.red);
		}
		else
		{
            R_Foot_Model.transform.position = new Vector3(R_Tracked_Object.position.x, frozenFloorHeight, R_Tracked_Object.position.z);
		}
        ...
        R_Foot_Model.eulerAngles = new Vector3(R_Foot_Model.eulerAngles.x, R_Tracked_Object.eulerAngles.y, R_Foot_Model.eulerAngles.z);
		...
		if (!isHeightTranslationFrozen && (Mathf.Abs (L_Foot_Height - R_Foot_Height) < noFallHeight)) 
        {
            ...
            {				
                transform.position = new Vector3(0, Mathf.Lerp(transform.position.y, L_Foot_Height - trackerDisplacement, speed), 0);
            } 
            else if (L_Foot_Height > R_Foot_Height + epsilon) // if right foot is lower
            {
                transform.position = new Vector3(0, Mathf.Lerp(transform.position.y, R_Foot_Height - trackerDisplacement, speed), 0);
            }
            else // else
            {
                transform.position = new Vector3(0, Mathf.Lerp(transform.position.y, L_Foot_Height - trackerDisplacement, speed), 0);
